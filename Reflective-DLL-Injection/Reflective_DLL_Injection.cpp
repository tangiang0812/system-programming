...
int main() {
  ...
  HANDLE dll = CreateFileA(
      "\\\\VBOXSVR\\Experiments\\MLLoader\\MLLoader\\x64\\Debug\\dll.dll",
  ...
  DWORD outSize = 0;
  ReadFile(dll, dllBytes, dllSize, &outSize, NULL);try keys are the most popular and common malware persistence mechanism used by threat actors.

  // get pointers to in-memory DLL headers
  PIMAGE_DOS_HEADER dosHeaders = (PIMAGE_DOS_HEADER)dllBytes;
  PIMAGE_NT_HEADERS ntHeaders =
      (PIMAGE_NT_HEADERS)((DWORD_PTR)dllBytes + dosHeaders->e_lfanew);
  SIZE_T dllImageSize = ntHeaders->OptionalHeader.SizeOfImage;

  // allocate new memory space for the DLL. Try to allocate memory in the
  // image's preferred base address, but don't stress if the memory is allocated
  // elsewhere
  // LPVOID dllBase = VirtualAlloc((LPVOID)0x000000191000000, dllImageSize,
  // MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  LPVOID dllBase =
      VirtualAlloc((LPVOID)ntHeaders->OptionalHeader.ImageBase, dllImageSize,
                   MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);

  // get delta between this module's image base and the DLL that was read into
  // memory
  ...
  for (size_t i = 0; i < ntHeaders->FileHeader.NumberOfSections; i++) {
    LPVOID sectionDestination =
        (LPVOID)((DWORD_PTR)dllBase + (DWORD_PTR)section->VirtualAddress);
    LPVOID sectionBytes =
        (LPVOID)((DWORD_PTR)dllBytes + (DWORD_PTR)section->PointerToRawData);
  ...
  while (relocationsProcessed < relocations.Size) {
    ...
    for (DWORD i = 0; i < relocationsCount; i++) {
      ...
      DWORD_PTR relocationRVA =
          relocationBlock->PageAddress + relocationEntries[i].Offset;
      DWORD_PTR addressToPatch = 0;
      ReadProcessMemory(GetCurrentProcess(),
                        (LPCVOID)((DWORD_PTR)dllBase + relocationRVA),
                        &addressToPatch, sizeof(DWORD_PTR), NULL);
      addressToPatch += deltaImageBase;
      std::memcpy((PVOID)((DWORD_PTR)dllBase + relocationRVA), &addressToPatch,
                  sizeof(DWORD_PTR));
    }
  }
      ...
      while (thunk->u1.AddressOfData != NULL) {
        if (IMAGE_SNAP_BY_ORDINAL(thunk->u1.Ordinal)) {
          ...
          PIMAGE_IMPORT_BY_NAME functionName =
              (PIMAGE_IMPORT_BY_NAME)((DWORD_PTR)dllBase +
  ...
  DLLEntry DllEntry = (DLLEntry)((DWORD_PTR)dllBase +
